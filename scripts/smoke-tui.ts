...
try {
  ...
  if (initialConfig.model?.main !== "bigmodel/glm-5.3"
    || initialConfig.provider?.bigmodel?.options?.apiKey !== undefined) {
...
if (plain.includes(`Update available! ${packageVersion} → ${availableVersion}`)) {
  throw new Error(`The TUI rendered a disabled cached update notice.\n${plain.slice(-4_000)}`);
}
...
if (!configured.includes(smokeApiKey)
  ...
  || !configured.includes('"lite": "bigmodel/glm-5.3-flash"')) {
