...
export function patchRuntimeLoginModelDefaults(runtime: string): string {
  const desiredPresetPattern = /[A-Za-z_$][\w$]*="zai\/glm-5\.3",[A-Za-z_$][\w$]*="zai\/glm-5\.3-flash",[A-Za-z_$][\w$]*="bigmodel\/glm-5\.3",[A-Za-z_$][\w$]*="bigmodel\/glm-5\.3-flash"/u;
  const desiredModelIdPattern = /[A-Za-z_$][\w$]*="glm-5\.3",[A-Za-z_$][\w$]*="glm-5\.3-flash"/u;
  const desiredCatalogMarker = 'name:"GLM-5.3-Flash"';
  const providerPruneMarker = '.filter(([e])=>e!=="zai"&&e!=="bigmodel")';
  if (desiredPresetPattern.test(runtime)
    && desiredModelIdPattern.test(runtime)
    && runtime.includes(desiredCatalogMarker)
    && runtime.includes(providerPruneMarker)) return runtime;

  const presetPattern = /([A-Za-z_$][\w$]*)="zai\/glm-(?:5\.1|5\.2|5\.3)",([A-Za-z_$][\w$]*)="zai\/glm-(?:4\.7|5-turbo|5\.3-flash)",([A-Za-z_$][\w$]*)="bigmodel\/glm-(?:5\.1|5\.2|5\.3)",([A-Za-z_$][\w$]*)="bigmodel\/glm-(?:4\.7|5\.3-flash)"/u;
  ...
  let patched = runtime
    .replace(
      preset[0],
      `${preset[1]}="zai/glm-5.3",${preset[2]}="zai/glm-5.3-flash",${preset[3]}="bigmodel/glm-5.3",${preset[4]}="bigmodel/glm-5.3-flash"`
    )
    .replace(modelIds[0], `${modelIds[1]}="glm-5.3",${modelIds[2]}="glm-5.3-flash"`)
    .replace(
      modelEntries[0],
      `models:{[${modelEntries[2]}]:{...${modelEntries[3]},name:"GLM-5.3"},[${modelEntries[4]}]:{...${modelEntries[5]},name:"GLM-5.3-Flash",limit:{context:1e6,output:128e3},modalities:{input:["text","image","video"],output:["text"]}},["glm-5-turbo"]:{...${modelEntries[1]}["glm-5-turbo"],name:"GLM-5-Turbo"}}`
    );

  const providerPattern = /\{\.\.\.([A-Za-z_$][\w$]*),provider:\{\.\.\.([A-Za-z_$][\w$]*),\[([A-Za-z_$][\w$]*)\]:\{/u;
  const provider = providerPattern.exec(patched);
  if (!provider) {
    throw new Error("ZCode runtime is incompatible with the login model defaults patch (provider anchor missing).");
  }
  patched = patched.replace(
    provider[0],
    `{...${provider[1]},provider:{...Object.fromEntries(Object.entries(${provider[2]}).filter(([e])=>e!=="zai"&&e!=="bigmodel")),[${provider[3]}]:{`
  );
...
const cliSourceTitleResolver = /function ([\w$]+)\(\)\{return process\.argv\.includes\("app-server"\)\|\|process\.argv\.includes\("agent-server"\)\?"electron":"cli"\}/u;
const desktopSourceTitleResolver = /function ([\w$]+)\(\)\{return"electron"\}/u;
const cliSourceTitleDefault = 'sourceTitle:e.sourceTitle??"cli"';
const desktopSourceTitleDefault = 'sourceTitle:e.sourceTitle??"electron"';

export function hasRuntimeDesktopSourceTitle(runtime: string): boolean {
  return desktopSourceTitleResolver.test(runtime)
    && runtime.includes(desktopSourceTitleDefault)
    && !runtime.includes(cliSourceTitleDefault);
}

export function patchRuntimeDesktopSourceTitle(runtime: string): string {
  if (hasRuntimeDesktopSourceTitle(runtime)) return runtime;
  if (!cliSourceTitleResolver.test(runtime) || !runtime.includes(cliSourceTitleDefault)) {
    throw new Error("ZCode runtime is incompatible with the Desktop source-title patch.");
  }
  return runtime
    .replace(cliSourceTitleResolver, 'function $1(){return"electron"}')
    .replace(cliSourceTitleDefault, desktopSourceTitleDefault);
}
...
export const runtimePatchPlan: readonly RuntimePatchDefinition[] = [
  {
    id: "desktop-source-title",
    requirement: "required",
    apply: patchRuntimeDesktopSourceTitle,
    verify: hasRuntimeDesktopSourceTitle
  },
  {
    id: "tui-bridge",
    requirement: "required",
