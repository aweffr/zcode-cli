...
import {
  ...
  patchRuntimeDesktopSourceTitle,
...
describe("runtime synchronization", () => {
  ...
  test("uses the Desktop source title for every official request path", () => {
    const runtime = [
      'function source(){return process.argv.includes("app-server")||process.argv.includes("agent-server")?"electron":"cli"}',
      'build({sourceTitle:e.sourceTitle??"cli"})'
    ].join("");

    const patched = patchRuntimeDesktopSourceTitle(runtime);
    expect(patched).toContain('function source(){return"electron"}');
    expect(patched).toContain('sourceTitle:e.sourceTitle??"electron"');
    expect(patchRuntimeDesktopSourceTitle(patched)).toBe(patched);
  });
  ...
  test("updates the login model defaults to the current server catalog", () => {
    ...
    expect(patched).toContain(
      'gni="zai/glm-5.3",_ni="zai/glm-5.3-flash",vni="bigmodel/glm-5.3",yni="bigmodel/glm-5.3-flash"'
    );
    ...
    expect(bigmodel.model).toEqual({ main: "bigmodel/glm-5.3", lite: "bigmodel/glm-5.3-flash" });
    expect(bigmodel.provider.bigmodel!.models["glm-5.3-flash"]).toBeDefined();
    expect(updateConfig(
      { model: { lite: "zai/custom-lite" } },
      "bigmodel",
      "bigmodel-key"
    ).model.lite).toBe("bigmodel/glm-5.3-flash");
    expect(updateConfig(
