...
describe("user config bootstrap", () => {
  ...
  test("recursively creates a private, credential-free config", async () => {
    ...
    const config = JSON.parse(await readFile(result.configPath, "utf8")) as {
      ...
      provider: { bigmodel: { options: { apiKey?: string }; models: Record<string, unknown> } };
      subagents: { autoBackgroundMs: number };
    };

    expect(result).toEqual({ configPath: userConfigPath(env), created: true });
    expect(Object.keys(config.provider)).toEqual(["bigmodel"]);
    expect(config.provider.bigmodel.options.apiKey).toBeUndefined();
    expect(config.provider.bigmodel.models["glm-5.3"]).toBeDefined();
    expect(config.provider.bigmodel.models["glm-5.3-flash"]).toBeDefined();
    expect(config.provider.bigmodel.models["glm-5-turbo"]).toBeDefined();
    expect(config.model).toEqual({ main: "bigmodel/glm-5.3", lite: "bigmodel/glm-5.3-flash" });
