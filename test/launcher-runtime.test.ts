...
describe("launcher/runtime integration", () => {
  test("removes telemetry exporters and disables update activity in the runtime process", async () => {
    const directory = await mkdtemp(join(tmpdir(), "zcode-launcher-network-policy-"));
    const fakeNode = join(directory, "fake-node");
    await writeFile(fakeNode, [
      "#!/bin/sh",
      "printf '%s|%s|%s|%s\\n' \"${ZCODE_MODEL_TELEMETRY_ENABLED:-}\" \"${ZCODE_DISABLE_UPDATE_CHECK:-}\" \"${NO_UPDATE_NOTIFIER:-}\" \"${OTEL_EXPORTER_OTLP_ENDPOINT:-}\"",
      ""
    ].join("\n"));
