...
test("recovers a real partial SSE connection reset without concatenating attempts", async () => {
  ...
  const defaultBigmodel = config.provider.bigmodel as { models: Record<string, unknown> };
  config.provider.bigmodel = {
    ...
    headers: {},
    models: defaultBigmodel.models
  };
  config.model = { main: "bigmodel/glm-5.3", lite: "bigmodel/glm-5.3" };
  config.storage = {
    dir: join(home, ".zcode"),
  ...
  try {
    ...
    const requestTitles = server.output().split("\n")
      .filter((line) => line.startsWith("X_TITLE "))
      .map((line) => JSON.parse(line.slice("X_TITLE ".length)) as string);

    expect(code, `${stderr}\nserver:\n${server.output()}${server.stderr()}`).toBe(0);
    expect(requestCount, stdout).toBe(2);
    expect(requestTitles).toEqual(["Z Code@electron", "Z Code@electron"]);
