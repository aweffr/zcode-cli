...
describe("launcher routing", () => {
  test("unconditionally disables telemetry and automatic update checks", () => {
    expect(applyCliNetworkPolicy({
      NO_UPDATE_NOTIFIER: "0",
      OTEL_EXPORTER_OTLP_ENDPOINT: "https://telemetry.example.com",
      OTEL_EXPORTER_OTLP_HEADERS: "authorization=secret",
      ZCODE_DISABLE_UPDATE_CHECK: "false",
  ...
  test("uses the extracted Desktop version for official ZCode request headers", async () => {
    const directory = await mkdtemp(join(tmpdir(), "zcode-app-version-"));
    const metadata = join(directory, "extraction.json");
    try {
      await writeFile(metadata, JSON.stringify({ appVersion: "3.10.2" }));
      expect(readRuntimeAppVersion(metadata)).toBe("3.10.2");
      expect(resolveZCodeAppVersion({}, metadata)).toBe("3.10.2");
      expect(resolveZCodeAppVersion({ ZCODE_APP_VERSION: " 3.10.1 " }, metadata)).toBe("3.10.1");
      await writeFile(metadata, JSON.stringify({ appVersion: "bad\u001b[2J" }));
      expect(readRuntimeAppVersion(metadata)).toBeUndefined();
      expect(resolveZCodeAppVersion({}, metadata)).toBeUndefined();
    } finally {
      await rm(directory, { recursive: true, force: true });
    }
  });
  ...
  test("clearing setup after a successful login is reflected in the wizard trigger", async () => {
    ...
    try {
      ...
      const config = JSON.parse(await readFile(configuredPath, "utf8")) as {
        provider?: { bigmodel?: { options?: { apiKey?: string } } };
      };
      config.provider!.bigmodel!.options!.apiKey = "login-written-key";
