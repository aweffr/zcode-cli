...
export function readRuntimeAppVersion(metadataPath = extractionMetadataPath): string | undefined {
  return readJsonVersion(metadataPath, "appVersion");
}

export function resolveZCodeAppVersion(
  env: NodeJS.ProcessEnv,
  metadataPath = extractionMetadataPath
): string | undefined {
  return safeVersion(env.ZCODE_APP_VERSION) ?? readRuntimeAppVersion(metadataPath);
}

export function applyCliNetworkPolicy(env: NodeJS.ProcessEnv): NodeJS.ProcessEnv {
  const isolated = { ...env };
  for (const key of Object.keys(isolated)) {
    if (key.toUpperCase().startsWith("OTEL_")) delete isolated[key];
  }
  return {
    ...isolated,
...
function runtimeEnvironment(extra: NodeJS.ProcessEnv = {}): Record<string, string> {
  ...
  const inherited = applyCliNetworkPolicy({
    ...env,
    ...extra
  });
  const appVersion = resolveZCodeAppVersion(inherited);
  const merged: NodeJS.ProcessEnv = {
    ...inherited,
    ZCODE_BASE_URL: resolveZCodeBaseUrl(inherited),
    ZCODE_MODEL_RETRY_MAX_RETRIES: resolveModelRetryMaxRetries(inherited),
    ...(appVersion ? { ZCODE_APP_VERSION: appVersion } : {}),
