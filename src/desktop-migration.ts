...
function desktopModels(provider: DesktopProviderConfig): DesktopModelMigration[] {
  ...
  for (const [id, raw] of Object.entries(provider.models ?? {})) {
    ...
    const detail = isRecord(raw) && typeof raw.name === "string" && raw.name.length > 0
      ? raw.name
      : id;
    models.push({ id: normalizedId, name: detail });
  }
  return models;
}
...
function mergeModels(
  ...
  for (const model of migrated) {
    const existing = isRecord(current?.[model.id]) ? current[model.id] : undefined;
...
function selectMainModel(
  ...
  return `${family}/${preferredOrFirstModel(candidateIds, previousMain?.split("/")[1] ?? previousMain, ["glm-5.3", "glm-5.2"])}`;
}
...
export async function applyDesktopMigration(
  ...
  await updateUserConfig((config) => {
    ...
    for (const knownFamily of knownFamilies) delete providers[knownFamily];
    providers[family] = {
      ...current,
      kind: "anthropic",
