...
[![release](https://img.shields.io/github/v/release/aweffr/zcode-cli)](https://github.com/aweffr/zcode-cli/releases)
...
Download the Linux amd64 or macOS arm64 archive from
[GitHub Releases](https://github.com/aweffr/zcode-cli/releases), extract it,
and run the included launcher. For example, on Linux:

```bash
tar -xzf zcode-app-cli-3.10.2-22-linux-amd64.tar.gz
cd zcode-app-cli-3.10.2-22-linux-amd64
./zcode
```

Keep the extracted directory intact; portable archives include Node.js.
...
Portable installations are updated by downloading and extracting a newer
GitHub Release. If you already have Node.js, install the release's npm tarball:

```bash
npm install -g ./zcode-app-cli-3.10.2-22.tgz
```

The public npm package `zcode-app-cli` belongs to the upstream project; this
repository distributes its builds through GitHub Releases only.

The CLI disables telemetry, update checks, and automatic updates. Upgrade it
only by running an explicit package-manager command such as the one above.
...
[github.com/aweffr/zcode-cli](https://github.com/aweffr/zcode-cli).
...
Based on [kingsword09/zcode-cli](https://github.com/kingsword09/zcode-cli).
